...
<x-guest-layout>
    ...
    <form method="POST" class="form-horizontal mt-3" action="{{ route('student.resetPasswordStore') }}">
        ...
        <div>
            ...
            <x-text-input id="email" class="form-control block mt-1 w-full" type="hidden" name="email" :value="old('email', !empty($studentEmail->email)? $studentEmail->email : 'Invalid token!')" required
